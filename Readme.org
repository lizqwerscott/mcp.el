...
To add JSON configurations for other clients, configure =mcp-hub-servers-config-path=. The default configuration file locations for Claude Desktop on macOS and Windows.
Then, call =mcp-hub-load-server-configs= to load these configurations into =mcp-hub-servers=.
#+begin_src elisp
  (add-to-list mcp-hub-servers-config-path
               "~/Library/Application Support/Claude/claude_desktop_config.json")

  (mcp-hub-load-server-configs)
#+end_src
